...
/**
 * A listener for NMS to Dynamic event.
 */
public class WorldEventListener implements IWorldEventListener{
    ...
    public void onEntityAdded(Entity entity) {
        if (entity instanceof EntityPlayer) {
            ...
            PlayerJoinEvent playerJoinEvent = new PlayerJoinEvent(new ImplementedPlayer(mpPlayer));
            try {
                EventManager.getManager().callEvent(playerJoinEvent);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
